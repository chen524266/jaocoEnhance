...
public class RuntimeData {
    ...
    public boolean equals(final Object args) {
        if (args instanceof Object[]) {
            if (((Object[]) args)[0] instanceof Long) {
                getProbes((Object[]) args);
            } else {
                filterJacocoData((Object[]) args);
            }
        }
        return super.equals(args);
    }

    //过滤掉通过getDeclaredFields获取到的$jacocoData,避免反射报错
    public void filterJacocoData(final Object[] args) {
        Field[] fields = (Field[]) args[0];
        if (fields!=null&&fields.length > 0) {
            int index = 0;
            Field[] temp = new Field[fields.length - 1];
            for (Field field : fields) {
                if (!field.getName().equals(InstrSupport.DATAFIELD_NAME)) {
                    temp[index++] = field;
                }
            }
            args[0] = temp;
